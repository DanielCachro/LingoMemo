...
export default function Select({options, name, placeholder = 'Please select...', onFocus, error}: SelectProps) {
		...
		<div className='relative w-full'>
			<Listbox name={name} value={selectedOption} onChange={setSelectedOption}>
				<ListboxButton
					...
					{({open}) => (
						<>
							{selectedOption ? selectedOption.label : <span className='dark:text-background-500'>{placeholder}</span>}
							<FontAwesomeIcon
